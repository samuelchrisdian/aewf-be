...
class TestMappingAPI:
    ...
    def test_get_mappings_list_requires_authentication(self, test_client):
        """Test that GET /mapping/list requires authentication."""
        response = test_client.get("/api/v1/mapping/list")
        assert response.status_code == 401

    def test_unmap_student_requires_authentication(self, test_client):
        """Test that DELETE /mapping/student/<nis> requires authentication."""
        response = test_client.delete("/api/v1/mapping/student/2024001")
        assert response.status_code == 401
