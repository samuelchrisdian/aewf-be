...
def import_attendance(current_user):
    ...
    try:
        ...
        result = IngestionService.import_logs_from_excel(
            filepath, filename, machine_code
        )
        os.remove(filepath)
        return jsonify({"success": True, "data": result}), 200
    except Exception as e:
...
@api_v1.route("/import/master/preview", methods=["POST"])
@token_required
def preview_master(current_user):
    """Preview master data from Excel file before import."""
    if "file" not in request.files:
        return jsonify({"message": "No file part"}), 400
    file = request.files["file"]
    if file.filename == "":
    ...
    try:
        filepath, _ = save_uploaded_file(file)
        result = MasterDataService.preview_from_excel(filepath)
        os.remove(filepath)
        return jsonify({"success": True, "data": result}), 200
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
...
@api_v1.route("/import/users-sync/preview", methods=["POST"])
@token_required
def preview_users_sync(current_user):
    """Preview machine users from Excel file before sync."""
    if "file" not in request.files:
        return jsonify({"message": "No file part"}), 400
    file = request.files["file"]
    if file.filename == "":
    ...
    try:
        filepath, _ = save_uploaded_file(file)
        result = MachineService.preview_users_from_excel(filepath, machine_code)
        os.remove(filepath)
        return jsonify({"success": True, "data": result}), 200
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
...
@api_v1.route("/import/attendance/preview", methods=["POST"])
@token_required
def preview_attendance(current_user):
    """Preview attendance logs from Excel file before import."""
    if "file" not in request.files:
        return jsonify({"message": "No file part"}), 400
    file = request.files["file"]
    if file.filename == "":
    ...
    try:
        filepath, filename = save_uploaded_file(file)
        result = IngestionService.preview_logs_from_excel(filepath, machine_code)
        os.remove(filepath)
        return jsonify({"success": True, "data": result}), 200
    except Exception as e:
        return jsonify({"success": False, "error": str(e)}), 500
...
def list_batches(current_user):
    ...
    batches, pagination = batch_service.list_batches(
        page=page, per_page=per_page, file_type=file_type, status=status
    )
...
def get_batch(current_user, batch_id):
    ...
    if error:
        return (
            jsonify(
                {"success": False, "error": {"code": "NOT_FOUND", "message": error}}
            ),
            404,
        )
    ...
    return (
        jsonify(
            {"success": True, "message": "Batch retrieved successfully", "data": batch}
        ),
        200,
    )
...
def delete_batch(current_user, batch_id):
    ...
    if not success:
        return jsonify(
            {"success": False, "error": {"code": "DELETE_FAILED", "message": error}}
