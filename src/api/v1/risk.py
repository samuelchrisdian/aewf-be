...
def get_student_risk(current_user, nis):
    ...
    return success_response(
        data=risk_data, message="Student risk retrieved successfully"
    )
...
def get_alerts(current_user):
    ...
    return paginated_response(
        data=alerts, pagination=pagination, message="Risk alerts retrieved successfully"
    )


@risk_bp.route("/alerts/actioned", methods=["GET"])
@token_required
def get_actioned_alerts(current_user):
    """
    ...
    Query Parameters:
        ...
        - start_date: Filter alerts from this date (format: YYYY-MM-DD)
        - end_date: Filter alerts until this date (format: YYYY-MM-DD)
        - page: Page number (default: 1)
        - per_page: Items per page (default: 20)

    Returns:
        Paginated list of actioned alerts
    """
    ...
    alerts, pagination = risk_service.get_actioned_alerts(
        class_id=class_id,
    ...
    return paginated_response(
        data=alerts,
        pagination=pagination,
        message="Actioned alerts retrieved successfully",
    )
...
def take_alert_action(current_user, alert_id):
    ...
    return success_response(message="Alert action recorded successfully")
...
def get_risk_history(current_user, nis):
    ...
    return success_response(data=history, message="Risk history retrieved successfully")
...
def recalculate_risks(current_user):
    ...
    results = risk_service.recalculate_risks(
        class_id=data.get("class_id"), student_nis=data.get("student_nis")
    )

    return success_response(data=results, message="Risk recalculation completed")
