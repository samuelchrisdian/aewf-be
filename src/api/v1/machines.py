...
def get_machines(current_user):
    ...
    sort_by, order = validate_sort_params(sort_by, order, ["machine_code", "location"])
...
def get_machine(current_user, machine_id):
    ...
    return success_response(data=machine_data, message="Machine retrieved successfully")
...
def create_machine(current_user):
    ...
    return created_response(data=machine_data, message="Machine created successfully")
...
def update_machine(current_user, machine_id):
    ...
    return success_response(data=machine_data, message="Machine updated successfully")
...
@machines_bp.route("/<int:machine_id>/users/<int:user_id>", methods=["DELETE"])
@token_required
def delete_machine_user(current_user, machine_id, user_id):
    """
    ...
    Returns:
        Success message
    """
    success, error = machine_service.delete_machine_user(machine_id, user_id)

    if not success:
        if "not found" in error.lower():
            return not_found_response("Machine user")
        return conflict_response(error)

    return success_response(message="Machine user deleted successfully")
