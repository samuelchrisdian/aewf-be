...
def get_daily_attendance(current_user):
    ...
    Query Parameters:
        ...
        - paginate: Enable/disable pagination (default: true, set to false to get all records)
    ...
    """
    # Get pagination params (now returns 3 values: page, per_page, paginate)
    page, per_page, should_paginate = get_pagination_params(request.args)
    ...
    result = attendance_service.get_daily_attendance(
        ...
        paginate_results=should_paginate,
    )

    # Return appropriate response based on pagination setting
    if should_paginate:
        ...
        return success_response(
            data=result["data"],
            message=f"Daily attendance retrieved successfully ({len(result['data'])} records)",
        )
...
def get_student_attendance(current_user, nis):
    ...
    attendance_data, error = attendance_service.get_student_attendance(
        nis=nis, start_date=start_date, end_date=end_date, month=month
    )
    ...
    return success_response(
        data=attendance_data, message="Student attendance retrieved successfully"
    )
...
def create_manual_attendance(current_user):
    ...
    return created_response(
        data=attendance_data, message="Attendance recorded successfully"
    )
...
def update_attendance(current_user, id):
    ...
    return success_response(
        data=attendance_data, message="Attendance updated successfully"
    )
...
def get_attendance_summary(current_user):
    ...
    summary_data = attendance_service.get_attendance_summary(
        class_id=class_id, period=period, start_date=start_date, end_date=end_date
    )

    return success_response(
        data=summary_data, message="Attendance summary retrieved successfully"
    )
