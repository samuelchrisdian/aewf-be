...
class AttendanceService:
    ...
    def get_daily_attendance(
        ...
        Args:
            ...
            paginate_results: Whether to paginate results (default: True)
        ...
        return {"data": attendance_data, "pagination": pagination}
    ...
    def get_student_attendance(
        ...
        records = self.repository.get_by_student(
            nis=nis, start_date=parsed_start, end_date=parsed_end
        )
            ...
            records_data.append(
                {
                    ...
                    "attendance_date": record.attendance_date.isoformat(),
                    "check_in": (
                        record.check_in.isoformat() if record.check_in else None
                    ),
                    "check_out": (
                        record.check_out.isoformat() if record.check_out else None
                    ),
                    "status": record.status,
                    "notes": record.notes,
                }
        ...
        # Detect patterns
        patterns = {"consecutive_absences": self._detect_consecutive_absences(records)}
    ...
    def create_manual_attendance(
        ...
        if self.repository.exists_for_date(
            validated_data["student_nis"], validated_data["attendance_date"]
        ):
            return None, {
                "attendance_date": ["Attendance record already exists for this date"]
            }
    ...
    def get_attendance_summary(
        ...
        stats = self.repository.get_summary_stats(
            class_id=class_id, start_date=parsed_start, end_date=parsed_end
        )
