...
class StudentService:
    ...
    def get_students(
        ...
        return {"data": students_data, "pagination": result["pagination"]}
    ...
    def create_student(self, data: dict) -> Tuple[Optional[dict], Optional[dict]]:
        ...
        if self.repository.exists(nis):
            # Check if it's a soft-deleted student
            inactive_student = self.repository.get_inactive_by_nis(nis)
            if inactive_student:
                return None, {
                    "nis": [
                        "A student with this NIS was previously deleted. "
                        "Use PUT /students/{nis} with is_active=true to reactivate."
                    ]
                }
            else:
