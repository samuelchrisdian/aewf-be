...
class StudentRepository:
    ...
    def exists(self, nis: str, active_only: bool = False) -> bool:
        """
        Check if a student with given NIS exists.

        Args:
            nis: Student NIS
            active_only: If True, only check for active students

        Returns:
            bool: True if exists
        """
        query = db.session.query(Student).filter(Student.nis == nis)
        if active_only:
            query = query.filter(Student.is_active == True)
        return db.session.query(query.exists()).scalar()

    def get_inactive_by_nis(self, nis: str) -> Optional[Student]:
        """
        Get an inactive (soft-deleted) student by NIS.

        Args:
            nis: Student NIS

        Returns:
            Inactive Student or None if not found/is active
        """
        return (
            db.session.query(Student)
            .filter(Student.nis == nis, Student.is_active == False)
            .first()
        )
    ...
    def get_attendance_summary(self, nis: str) -> dict:
        ...
        records = (
            db.session.query(AttendanceDaily)
