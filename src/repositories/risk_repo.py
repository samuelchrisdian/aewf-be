...
from src.domain.models import (
    RiskAlert, RiskHistory, Student, Class, Teacher, AttendanceDaily
)
...
class RiskRepository:
    ...
    def get_at_risk_students(
        ...
        try:
            ...
            for risk_history, student_name, student_class_id, class_name in results:
                ...
                alert = session.query(RiskAlert).filter(
                ...
                students.append({
                    ...
                    "last_updated": risk_history.calculated_at.isoformat() if risk_history.calculated_at else None,
    ...
    def get_student_risk(self, nis: str) -> Optional[dict]:
        ...
        try:
            ...
            risk_history = session.query(RiskHistory).filter(
                RiskHistory.student_nis == nis
            ).order_by(desc(RiskHistory.calculated_at)).first()
            
            # Get class info
            class_info = session.query(Class).filter(
                Class.class_id == student.class_id
            ).first()
            
            # Check for active alert
            alert = session.query(RiskAlert).filter(
                RiskAlert.student_nis == nis,
                RiskAlert.status == 'pending'
            ).first()
            ...
            return {
                ...
                "last_updated": risk_history.calculated_at.isoformat() if risk_history else None,
                "alert_generated": alert is not None
            }
    ...
    def get_alerts(
        ...
        try:
            query = session.query(
                ...
                Student, RiskAlert.student_nis == Student.nis
            ).outerjoin(
                Class, Student.class_id == Class.class_id
            ).outerjoin(
                Teacher, RiskAlert.assigned_to == Teacher.teacher_id
            )
            ...
            # Format results
            alerts = []
            for alert, student_name, student_class_id, class_name, assignee_name in results:
                alerts.append({
                    "id": alert.id,
                    ...
                    "follow_up_date": alert.follow_up_date.isoformat() if alert.follow_up_date else None,
    ...
    def get_risk_history(
        ...
        session = SessionLocal()
        try:
            records = session.query(RiskHistory).filter(
                RiskHistory.student_nis == nis
            ).order_by(desc(RiskHistory.calculated_at)).limit(limit).all()
