...
class MachineRepository:
    ...
    def get_user_count(self, machine_id: int) -> int:
        ...
        """
        return (
            db.session.query(func.count(MachineUser.id))
            .filter(MachineUser.machine_id == machine_id)
            .scalar()
            or 0
        )
    ...
    def get_machine_user_by_id(self, user_id: int) -> Optional[MachineUser]:
        """
        ...
        Returns:
            MachineUser or None if not found
        """
        return db.session.query(MachineUser).filter(MachineUser.id == user_id).first()

    def delete_machine_user(self, user_id: int) -> bool:
        """
        Delete a machine user. Related mappings and raw logs will be cascade deleted.
        ...
        Returns:
            bool: True if deleted, False if not found
        """
        user = self.get_machine_user_by_id(user_id)
        if not user:
            return False

        # Delete related mapping if exists
        if user.student_map:
            db.session.delete(user.student_map)

        # Delete related raw logs
        for log in user.raw_logs:
