...
def get_pagination_params(request_args: dict) -> tuple:
    ...
    Returns:
        tuple: (page, per_page, paginate)
        - paginate: True by default, False if paginate=false is passed
    """
    # Check if pagination is disabled
    paginate_param = request_args.get("paginate", "true").lower()
    paginate = paginate_param != "false"
...
def get_all_from_query(query):
    """
    Get all results from a query without pagination.

    Args:
        query: SQLAlchemy query object

    Returns:
        dict: {"data": [...], "pagination": None}
    """
    items = query.all()
    return {"data": items, "pagination": None}
